...
The `todo-back` quickstart demonstrates how to implement a backend that exposes a Rest API with `JAX-RS`
to manage a list of ToDo which are persisted in a database with `JPA`.
...
* TBD
* mention wildfly-datasources-feature-pack and the env vars

=== CORS Filters

* TBD
* Mention the CORS filters and the src/main/resources/scripts/cors_filters.cli script
